...
% as it would require the entire dataset to be loaded into memory, and would in turn be very slow. % This is because the generator would have to iterate through the entire dataset for every batch, and would have to produce every possible combination of images for every batch. % This is not a viable solution, and thus a different approach was needed.
...
% TODO Change image
\begin{figure} % Single column figure
	\includegraphics[width=\linewidth]{Triplet_Margin.png}
...
\begin{figure*}[hbt!] % Single column figure
	\centering
    \includegraphics[width=0.6\linewidth]{Forced.png}
...
% https://stackoverflow.com/questions/5647118/is-it-possible-to-do-partial-inheritance-with-python
...
% image_dataset_from_directory(Data_Structure.triplet_loss)
% def (Data_Structure.contrastive_loss)
% 
% class Data_Structure(object):
%     def __init__(self, learning_size, classification: (learning_size, int), shuffle: boolean = True):
%         self.name = name
%         self.s1 = s1
%         self.s2 = s2
%         self.s3 = s3
...
%         return 0
% 
% class contrastive(Data_Structure):
%     def calculate(self):
%         self.pm = 0
...
%         return self.pm
% 
% def image_dataset_from_directory(structure: Type[tf.Data_Structure] = tf.Data_Structure.contrastive)
%     structure.calculate()
